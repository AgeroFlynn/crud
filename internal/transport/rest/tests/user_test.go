...
func (ut *UserTests) postUser400(t *testing.T) {
	body, err := json.Marshal(&incoming.NewUser{})
	if err != nil {
		t.Fatal(err)
	}
...
func (ut *UserTests) postUser403(t *testing.T) {
	body, err := json.Marshal(&incoming.User{})
	if err != nil {
		t.Fatal(err)
	}
...
func (ut *UserTests) postUser201(t *testing.T) incoming.User {
	nu := incoming.NewUser{
		Name:            "Bill Kennedy",
